...
export function ScholarPanel({ locale, methodologies }: { locale: Locale; methodologies: Methodology[] }) {
  ...
  if (!user?.scholar) return <div className="scholar-gate"><span className="auth-required-mark">✦</span><p className="section-kicker">{isBangla ? 'অনুমোদিত স্কলারদের জন্য' : 'For approved scholars'}</p><h1>{isBangla ? 'আপনার স্কলার প্যানেল এখনও সক্রিয় নয়' : 'Your scholar panel is not active yet'}</h1><p>{isBangla ? 'অ্যাডমিন আপনার স্কলার প্রোফাইল অনুমোদন করলে এখানে নির্ধারিত প্রশ্ন ও পর্যালোচনা দেখতে পাবেন।' : 'Once an administrator approves your scholar profile, assigned questions and independent reviews will appear here.'}</p><Link className="button" href={`/${locale}/account`}>{isBangla ? 'অ্যাকাউন্টে ফিরুন' : 'Back to account'}</Link></div>;
